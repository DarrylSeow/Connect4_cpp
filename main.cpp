...
void settings() {
    ...
    do {
        ...
        if (user_input == "/back") {
            ...
            string new_diff;
            cout << "Difficulty can only range from 1 to 3, with 1 being the easiest and 3 being the hardest.\n";
            cout << "Please input the difficulty level of the CPU player.\n";
...
void cpu_move() {
    if (cpu_difficulty == 1) {
        cpu1(); // use level 1 gyatt to make a move
    } else if (cpu_difficulty == 2) {
        cpu2(); // use level 2 gyatt to make a move
    }
}
